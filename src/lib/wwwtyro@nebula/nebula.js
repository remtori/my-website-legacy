...
export function generateNoiseTexture(regl, rng, size) {
  ...
  for (let i = 0; i < l; i++) {
    ...
    array[i * 2 + 0] = Math.round(0.5 * (1.0 + Math.cos(r)) * 255);
    array[i * 2 + 1] = Math.round(0.5 * (1.0 + Math.sin(r)) * 255);
  }
...
export function createRenderer(regl, rng) {
  let pgWidth = 256;
  let pgTexture = generateNoiseTexture(regl, rng, pgWidth);
