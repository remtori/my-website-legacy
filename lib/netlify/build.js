...
async function build() {
	...
	if (process.env.NETLIFY) {
		console.log(`Pulling latest content from my-website-content`);
		const cwdDir = process.cwd();
		process.chdir(config.rootDir);
		await fs.remove('./site-content');
		await execute(`git clone https://github.com/remtori/my-website-content.git site-content`).then(console.log);
	...
	process.env.NETLIFY && await update_published_commit();
