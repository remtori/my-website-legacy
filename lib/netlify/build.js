...
async function build() {
	...
	if (process.env.NETLIFY) {
		...
		cwdUtils.push(config.rootDir);
		await fs.remove(config.contentDir);
		console.log(await execute(
			`git clone https://github.com/remtori/my-website-content.git ${path.basename(config.contentDir)}`
		));
		cwdUtils.pop();
	}
